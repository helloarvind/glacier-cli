...
/**
 * A command line client to Amazon Glacier based on AWS examples. http://aws.amazon.com/glacier
 * 
 * Uses Glacier high level API for uploading, downloading, deleting files, and the low-level one for retrieving vault
 * inventory.
 * 
...
public class Glacier {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      ...
      formatter.printHelp("glacier " + "upload vault_name file1 file2 ... | "
          + "download vault_name archiveId output_file | " + "delete vault_name archiveId | " + "inventory vault_name",
          options);
    }
  ...
  public void upload(String vaultName, String archive) {
    ...
    try {
      ...
      System.out.println("Uploaded " + archive + ": " + result.getArchiveId() + "at: " + new Date());
      FileWriter upLoadedArchiveList = new FileWriter("uploaded-archive.csv", true);
      upLoadedArchiveList.write(vaultName + "," + archive + "," + result.getArchiveId() + "," + new Date());
  ...
  private QueueConfig setupSNS(QueueConfig config, String snsTopicName) {
    ...
    SubscribeRequest request2 = new SubscribeRequest().withTopicArn(config.snsTopicARN)
        .withEndpoint(config.sqsQueueARN).withProtocol("sqs");
  ...
  private Boolean waitForJobToComplete(String jobId, String sqsQueueUrl) throws InterruptedException,
    ...
    while (!messageFound) {
      ...
      List<Message> msgs = sqsClient.receiveMessage(new ReceiveMessageRequest(sqsQueueUrl).withMaxNumberOfMessages(10))
          .getMessages();
